...
public class Printer {
	...
	private static void print(NbtType type, int indent) {
		if (type.getId() == NbtTag.NBT_TAG_COMPOUND) {
		...
		} else if (type.getId() == NbtTag.NBT_TAG_LIST) {
			String tab = new String(new char[indent]).replace("\0", "\t");
			...
			System.out.println(tab + "}");
		} else if (type.getId() == NbtTag.NBT_TAG_BYTE_ARRAY) {
		...
		} else if (type.getId() == NbtTag.NBT_TAG_INT_ARRAY) {
		...
		} else if (type.getId() == NbtTag.NBT_TAG_LONG_ARRAY) {
